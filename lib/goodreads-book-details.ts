import { fetchWithConfig } from "@/lib/api-config";
...
export function normalizePublicationDate(raw: string): string | null {
...
function stripContributorRole(
  name: string,
  role: "translator" | "illustrator"
): string {
...
function parseApolloBookDetails(
  ...
  const isbn10 = isbn10Raw && isbn10Raw.length === 10 ? isbn10Raw : null;
  ...
  const type = typeof format === "string" && format.trim() ? format.trim() : null;
  ...
  const asin = typeof asinRaw === "string" && asinRaw.trim() ? asinRaw.trim() : null;
...
function findBookInApolloState(
  ...
  for (const key of Object.keys(apolloState)) {
    ...
    const book = node as { __typename?: string; legacyId?: number };
    if (book.__typename === "Book" && String(book.legacyId) === legacyId) {
...
async function fetchParseableBookHtml(scrapeURL: string): Promise<string> {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const blocked = isLikelyBlockedOrChallengeHtml(htmlString);
...
export async function scrapeBookDetails(slug: string, includeReviews = false) {
  ...
  const apolloDetails = legacyId ? parseApolloBookDetails(htmlString, legacyId) : null;
  const pages =
    apolloDetails?.pages ?? (pagesMatch ? parseInt(pagesMatch[1], 10) : null);
  ...
  const contributors = $(".ContributorLinksList > span > a")
    .map((i: number, el: any) => {
      ...
      return {
        id: i + 1,
        name: $el.find("span").text().replace(/\s+/g, " ").trim(),
        url: ($el.attr("href") || "").replace("https://www.goodreads.com", ""),
      };
    })
    .toArray();
  ...
  const genres = $('[data-testid="genresList"] > ul > span > span')
    .map((_: number, el: any) => $(el).find("span").text().replace("Genres", "").trim())
    .get()
    .filter((genre: string) => genre && genre.length > 0);
  const bookEdition = extractBookEditionFormat(pagesText, apolloDetails?.type);
  const publishDate =
    apolloDetails?.publishDate ??
    normalizePublicationDate($('[data-testid="publicationInfo"]').text());
  ...
  const related = $("div.DynamicCarousel__itemsArea > div > div")
    .map((i: number, el: any) => {
      ...
      return {
        id: i + 1,
        src: $el.find("div > a > div:nth-child(1) > div > div > img").attr("src"),
        title: $el.find('div > a > div:nth-child(2) > [data-testid="title"]').html(),
        author: $el.find('div > a > div:nth-child(2) > [data-testid="author"]').html(),
        url: ($el.find("div > a").attr("href") || "").replace(
  ...
  const reviews = includeReviews
    ? $(".ReviewsList > div:nth-child(2) > div")
        ...
        .map((i: number, el: any) => {
          ...
          return {
            id: i + 1,
            image: $el.find("div > article > div > div > section > a > img").attr("src"),
            author: $el
              .find(
